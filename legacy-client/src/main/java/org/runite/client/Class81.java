...
final class Class81 {
    ...
    static void friendsIgnoreListAlerts(long var1) {
        try {
            if (var1 != 0L) {
                if (Class3_Sub28_Sub5.ignoreCount < 100) {
                    RSString var4 = Objects.requireNonNull(Unsorted.decodeBase37(var1)).longToRSString();

                    int var5;
                    for (var5 = 0; var5 < Class3_Sub28_Sub5.ignoreCount; ++var5) {
                        if (Class114.ignoreList[var5] == var1) {
                            BufferedDataStream.addChat(RSString.parse(""), 0, RSString.stringCombiner(new RSString[]{var4, TextCore.HasIgnoreAlready}), -1);
                            return;
                        }
                    }

                    for (var5 = 0; Class8.totalFriendCount > var5; ++var5) {
                        if (var1 == Class50.friendNameLong[var5]) {
                            BufferedDataStream.addChat(RSString.parse(""), 0, RSString.stringCombiner(new RSString[]{TextCore.HasPleaseRemove, var4, TextCore.HasFriendsToIgnore}), -1);
                            return;
                        }
                    }

                    if (var4.equalsString(Class102.player.displayName)) {
                        BufferedDataStream.addChat(RSString.parse(""), 0, TextCore.HasOnOwnIgnoreList, -1);
                    } else {
                        Class114.ignoreList[Class3_Sub28_Sub5.ignoreCount] = var1;
                        TextureOperation7.aStringArray3341[Class3_Sub28_Sub5.ignoreCount++] = Unsorted.decodeBase37(var1);
                    ...
                    BufferedDataStream.addChat(RSString.parse(""), 0, TextCore.HasIgnoreListFull, -1);
                }
            }
        } catch (RuntimeException var6) {
