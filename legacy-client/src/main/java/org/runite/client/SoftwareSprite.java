...
public class SoftwareSprite extends AbstractSprite {
   ...
   public void drawMinimapRegion(int x, int y, int w, int h, int anchorX, int anchorY, int theta, int zoom, int[] lineStart, int[] lineWidth) {
      try {
         int centerX = -w / 2;
         int centerY = -h / 2;
         ...
         int origin = x + y * Toolkit.JAVA_TOOLKIT.width;
         ...
         for (y = 0; y < h; ++y) {
            ...
            for (x = -lineWidth[y]; x < 0; ++x) {
               ...
               if (GameConfig.MINIMAP_FILTER) {
                  ...
                  int x2 = x1 + 1;
                  int y2 = y1 + 1;
                  int sampleColor1 = this.anIntArray4081[x1 + y1 * this.width];
                  int sampleColor2 = this.anIntArray4081[x2 + y1 * this.width];
                  int sampleColor3 = this.anIntArray4081[x1 + y2 * this.width];
                  int sampleColor4 = this.anIntArray4081[x2 + y2 * this.width];
                  ...
                  Toolkit.JAVA_TOOLKIT.getBuffer()[dstOff++] = red | green | blue;
               } else {
                  Toolkit.JAVA_TOOLKIT.getBuffer()[dstOff++] = this.anIntArray4081[(dstX >> 16) + (dstY >> 16) * this.width];
               }
               dstX += cos;
               dstY -= sin;
            }
            ...
            originX += sin;
            originY += cos;
            origin += Toolkit.JAVA_TOOLKIT.width;
         }
      } catch (Exception ex) {
      }
   }
   ...
   public void drawCompass(int x, int y, int w, int h, int anchorX, int anchorY, int theta, int[] lineStart, int[] lineWidth) {
      try {
         int centerX = -w / 2;
         int centerY = -h / 2;
         int sin = (int)(Math.sin((double)theta / 326.11D) * 65536.0D);
         ...
         for (y = 0; y < h; ++y) {
            ...
            for (x = -lineWidth[y]; x < 0; ++x) {
               int color = this.anIntArray4081[(dstX >> 16) + (dstY >> 16) * this.width];
               if (color == 0) {
                  ++dstOff;
               } else {
                  // Bilinear filtering:
                  if (GameConfig.MINIMAP_FILTER) {
                     ...
                     Toolkit.JAVA_TOOLKIT.getBuffer()[dstOff++] = color;
                  }
               }
