...
final class ItemDefinition {
	...
	static int[] levelxpLookup = new int[99];
	int teamId = 0;
	...
	static AssembledMethod getCS2ById(int methodID) {
	   try {
	...
	private void parseOpcode(DataBuffer buffer, int opcode) {
		try {
			if(opcode == 1) {
				...
				this.iconScale = buffer.readUnsignedShort();
			} else if (opcode == 5) {
				this.iconPitch = buffer.readUnsignedShort();
			} else if (opcode == 6) {
				this.iconYAw = buffer.readUnsignedShort();
			} else if (opcode == 7) {
	...
	final void method1118(ItemDefinition var1, ItemDefinition var2) {
		try {
			this.name = var1.name;
			this.iconScale = var2.iconScale;

			this.aShortArray774 = var2.aShortArray774;
			this.aShortArray772 = var2.aShortArray772;
			this.iconPitch = var2.iconPitch;
			this.iconYAw = var2.iconYAw;
			this.aShortArray751 = var2.aShortArray751;
