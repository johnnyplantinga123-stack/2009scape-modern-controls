    ...
    public static void ClientCommands(RSString command) {
        try {
            ...
            if (command.equalsStringIgnoreCase(TextCore.COMMAND_GARBAGE_COLLECTOR)) {
                ...
                BufferedDataStream.addChat(null, 0, RSString.stringCombiner(new RSString[]{TextCore.memoryEquals, RSString.stringAnimator(var3), TextCore.Memoryk}), -1);
            }
            ...
            if (command.equalsStringIgnoreCase(TextCore.COMMAND_MEMORY_MANAGEMENT)) {
                ...
                runtime = Runtime.getRuntime();
                var3 = (int) ((runtime.totalMemory() + -runtime.freeMemory()) / 1024L);
                BufferedDataStream.addChat(null, 0, RSString.stringCombiner(new RSString[]{TextCore.memoryBeforeCleanup, RSString.stringAnimator(var3), TextCore.Memoryk}), -1);
                ...
                var3 = (int) ((runtime.totalMemory() + -runtime.freeMemory()) / 1024L);
                BufferedDataStream.addChat(null, 0, RSString.stringCombiner(new RSString[]{TextCore.aString_2033, RSString.stringAnimator(var3), TextCore.Memoryk}), -1);
            }

            if (command.equalsStringIgnoreCase(TextCore.COMMAND_PC_CACHE_SIZE)) {
                BufferedDataStream.addChat(null, 0, RSString.stringCombiner(new RSString[]{TextCore.aString_442, RSString.stringAnimator(Unsorted.method1727((byte) 123))}), -1);
            }
            ...
            if (command.equalsStringIgnoreCase(TextCore.TOGGLE_FK)) {
                ...
                modernHotkeys = true;
                BufferedDataStream.addChat(null, 0, RSString.parse("Modern hotkeys mode toggled " + (on ? "on." : "off.")), -1);
            }
            ...
            if (command.startsWith(TextCore.COMMAND_RECT_DEBUG)) {
                ...
                BufferedDataStream.addChat(null, 0, RSString.stringCombiner(new RSString[]{TextCore.rectDebugEquals, RSString.stringAnimator(Client.rectDebugInt)}), -1);
            }
            ...
            if (command.equalsStringIgnoreCase(TextCore.COMMAND_TWEENING)) {
                if (tweeningEnabled) {
                    ...
                    BufferedDataStream.addChat(null, 0, TextCore.forcedTweeningDisabled, -1);
                } else {
                    tweeningEnabled = true;
                    BufferedDataStream.addChat(null, 0, TextCore.forcedTweeningEnabled, -1);
                }
            }
            ...
            if (command.startsWith(TextCore.COMMAND_GETBITS)) {
                ...
                try {
                    ...
                    writer.flush();
                    writer.close();
                    BufferedDataStream.addChat(null, 0, RSString.parse("Wrote bits to " + file.getAbsolutePath()), -1);
                } catch (Exception e){
