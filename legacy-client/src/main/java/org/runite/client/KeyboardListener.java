   ...
   public final void keyTyped(KeyEvent var1) {
      ...
      if(var1.isAltDown()){
         if(var1.getKeyChar()=='n'){
            ...
            BufferedDataStream.addChat(RSString.parse("<img=2>Client debug notifier"), 0, RSString.parse("NPC debug context " + (GameConfig.NPC_DEBUG_ENABLED ? "enabled." : "disabled.")), -1);
            return;
         }
         if(var1.getKeyChar()=='o'){
            GameConfig.OBJECT_DEBUG_ENABLED = !GameConfig.OBJECT_DEBUG_ENABLED;
            BufferedDataStream.addChat(RSString.parse("<img=2>Client debug notifier"), 0, RSString.parse("Object debug context " + (GameConfig.OBJECT_DEBUG_ENABLED ? "enabled." : "disabled.")), -1);
            return;
         }
         if(var1.getKeyChar()=='i'){
            GameConfig.ITEM_DEBUG_ENABLED = !GameConfig.ITEM_DEBUG_ENABLED;
            BufferedDataStream.addChat(RSString.parse("<img=2>Client debug notifier"), 0, RSString.parse("Item debug context " + (GameConfig.ITEM_DEBUG_ENABLED ? "enabled." : "disabled.")), -1);
            return;
         }
         return;
      }
