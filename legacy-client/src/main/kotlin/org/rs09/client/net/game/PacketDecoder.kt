...
//        println("NOTE > Registered ConsoleAutocompletionPacket as opcode $completion")
    ...
    fun decodePacket(): Boolean {
        ...
        if (Unsorted.packetOpcode == -1) {
            ...
            Unsorted.packetOpcode = BufferedDataStream.incomingBuffer.opcode
            Unsorted.packetSize = Class75_Sub4.incomingPacketSizes[Unsorted.packetOpcode]
        }

        if (Unsorted.packetSize == -1) {
            if (availableBytes < 1) return false

            availableBytes--
            Class3_Sub15.activeConnection.readBytes(BufferedDataStream.incomingBuffer.buffer, 0, 1)
            Unsorted.packetSize = BufferedDataStream.incomingBuffer.buffer[0].toInt() and 0xff
        }

        if (Unsorted.packetSize == -2) {
            if (availableBytes < 2) return false

            availableBytes -= 2
            Class3_Sub15.activeConnection.readBytes(BufferedDataStream.incomingBuffer.buffer, 0, 2)
            BufferedDataStream.incomingBuffer.index = 0
            Unsorted.packetSize = BufferedDataStream.incomingBuffer.readUnsignedShort()
        }

        if (availableBytes < Unsorted.packetSize) return false

        BufferedDataStream.incomingBuffer.index = 0
        Class3_Sub15.activeConnection.readBytes(BufferedDataStream.incomingBuffer.buffer, 0, Unsorted.packetSize)
        Class24.anInt469 = Class7.anInt2166
        Class7.anInt2166 = LinkableRSString.anInt2582
        LinkableRSString.anInt2582 = Unsorted.packetOpcode
        AbstractSprite.anInt3699 = 0

        val decoder = decoders[Unsorted.packetOpcode]
