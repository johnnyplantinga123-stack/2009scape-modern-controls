...
public final class Keyboard implements KeyListener, FocusListener {
	...
	public final synchronized void keyPressed(@OriginalArg(0) KeyEvent event) {
		...
		// F11 camera-mode cycling must be handled at the AWT boundary so the
		// client responds once per physical key press even when interface
		// scripts later consume key events. This is the single authoritative
		// toggle path (edge-triggered by the key-pressed event).
		CameraMode.onKeyPressed(code);
