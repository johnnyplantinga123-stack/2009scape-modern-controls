...
object SystemLogger {
    ...
    @JvmStatic
    fun initTradeLogger(logPath: String){
        if(!File(logPath).exists()){
            File(logPath).mkdirs()
        }
        tradeLog = FileWriter(logPath + File.separator + SimpleDateFormat("dd-MM-yyyy").format(System.currentTimeMillis()) + ".log")
